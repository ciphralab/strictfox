...
2. **Secure DNS**: Browser-level DNS-over-HTTPS (DoH) via Quad9 is **enabled by default** in this configuration.
   - *Alternative:* If you prefer to handle DNS at the system/Network level comment out `[SECTION 0705]` here [user.js](user.js)
...
3. Place the `user.js` file directly into this directory. If a `user.js` file already exists, back it up before overwriting or merge the configurations manually.
   - *Optional:* If you use system/network-level DNS, open the [user.js](user.js) file and comment out every line in `[SECTION 0705]`
...
- **Network Hardening**: Disables DNS prefetching, speculative connections, and WebRTC IP leakage. Enforces HTTPS-Only mode globally. Browser-level DNS-over-HTTPS (DoH) via Quad9 is enabled by default, with a seamless fallback to system/network DNS if `[SECTION 0705]` is commented out.
